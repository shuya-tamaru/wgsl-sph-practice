export class Particles {
  ...
  createParticleData(
    ...
    this.positions = new Float32Array(particleCount * 3); // xyz
    ...
    for (let i = 0; i < this.xCount; i++) {
      for (let j = 0; j < this.yCount; j++) {
        for (let k = 0; k < this.zCount; k++) {
          ...
          // const nx = this.xCount > 1 ? (i / (this.xCount - 1)) * 2.0 - 1.0 : 0;
          // const ny = this.yCount > 1 ? (j / (this.yCount - 1)) * 2.0 - 1.0 : 0;
          // const nz = this.zCount > 1 ? (k / (this.zCount - 1)) * 2.0 - 1.0 : 0;
          // // 法線ベクトルを正規化
  ...
  createBuffers(device: GPUDevice) {
    ...
    this.positionBufferLayout = {
      arrayStride: 12, // 3 floats for xyz
      attributes: [
        {
          shaderLocation: 0,
          format: "float32x3",
