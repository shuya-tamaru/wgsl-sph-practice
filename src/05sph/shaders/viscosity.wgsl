...
@group(0) @binding(3) var<uniform> sp : SphParams;

@group(0) @binding(4) var<uniform> gp : GridParams;

@group(0) @binding(5) var<storage, read> srcVelocities : array<vec4<f32>>;

@group(0) @binding(6) var<storage, read_write> dstVelocities : array<vec4<f32>>;

@group(0) @binding(7) var<uniform> ts : TimeStep;

fn laplacianW_viscosity(r: f32) -> f32 {
...
fn cs_main(@builtin(global_invocation_id) global_id: vec3<u32>) {
  ...
  let vi   = srcVelocities[i].xyz;
  let rhoi = max(density[i], 1e-6); // guard

  // 近傍探索（27セル）
  let ci = pos_to_cell_coord(pi);
  var a_visc = vec3<f32>(0.0);
      ...
      for (var dx: i32 = -1; dx <= 1; dx++) {
        ...
        loop {
          ...
          if (j != i) {
            ...
            let vj   = srcVelocities[j].xyz;
            let rhoj = max(density[j], 1e-6);

            let rvec = pi - pj;
            let r    = length(rvec);

            if (r > 0.0 && r < sp.h) {
              let lap = laplacianW_viscosity(r);
              // a_visc += μ * m * (vj - vi) / ρj * ∇²W
              a_visc += sp.viscosity * sp.mass * (vj - vi) / rhoj * lap;
            }
          }
          j = j + 1u;
        }
